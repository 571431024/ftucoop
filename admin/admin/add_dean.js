...
$(document).ready(function(){
    ...
    $("#tb_dean_faculty").on('change', function(){
        getMajorByFaculty();
    });
});
// End Auto Run jQuery
...
function getMajorByFaculty() {
    ...
    var faculty = $("#tb_dean_faculty").val();
    var data = {
        db : "tb_major",
        name : faculty
    }
    $.ajax({
        ...
        success: function(result){
            ...
            console.log(result);
            $("#tb_dean_major").find('option').remove();
            $("#tb_dean_major").html('<option value="no">เลือกสาขา</option>'); 
            if(result.length > 0 ){
                for (var i = 0; i <= result.length; i++) {
                    var option = "<option value='" + result[i].tb_major_name + "'>" + result[i].tb_major_name + "</option>";
                    $("#tb_dean_major").append(option);
                }
            }
        }
    });
