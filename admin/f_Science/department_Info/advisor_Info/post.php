	...
	*/
	if (!isset($FORM_ERRORS)) {
		...
		*/
		...
		$sql8 = "SELECT * FROM tb_major WHERE tb_major_name ='$major'"; 
		$query8=mysql_query($sql8);
		$data8=mysql_fetch_array($query8);
		// $rows8=mysql_num_rows($query8);
		echo $data8['tb_major_id'];
		$mysqli->query(
			/*
			...
			*/
			...
			(
				...
				'{$_SERVER['REMOTE_ADDR']}',
				'{$mysqli->escape_string($data8['tb_major_id'])}',
			)
			"
		);
		/*
		...
		*/
		// header('Location: webboard.php?highlight_id=' . $mysqli->insert_id);
		/*
		จบการทำงาน
		*/
		exit;
